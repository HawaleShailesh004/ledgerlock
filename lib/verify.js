...
export async function verifyChain(tenantId) {
  ...
  // sealed, straight from live data. If anything inside that window changed, the
  // live root diverges from the immutable checkpoint root.
  const checkpoint = await getCheckpoint(tenantId);
  const boundary = checkpoint ? checkpoint.count : res.Items.length;
  const liveRootAtBoundary = merkleRoot(
    res.Items.slice(0, boundary).map((it) => it.hash)
  );

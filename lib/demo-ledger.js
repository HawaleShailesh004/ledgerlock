...
// Binary Merkle root over ordered leaf hashes — matches lib/merkle.js
...
export const ACTIONS = ["PHI_READ", "RECORD_UPDATE", "EXPORT", "BREAK_THE_GLASS"];
...
export async function buildSeed(tenantId) {
  ...
  for (let seq = 0; seq < TOTAL; seq++) {
    ...
    const base = {
      ...
      actor,
      action,
      payload: { ip: `10.0.${1 + Math.floor(rng() * 40)}.${1 + Math.floor(rng() * 250)}` },
...
export async function buildCheckpoint(events) {
  const count = Math.floor(events.length / CHECKPOINT_INTERVAL) * CHECKPOINT_INTERVAL;
...
export async function verifyDemo(events, checkpoint) {
  ...
  const liveRootAtBoundary = await merkleRoot(recomputedHashes.slice(0, boundary));
  return {
